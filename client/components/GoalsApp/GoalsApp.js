...
class GoalsApp extends React.Component {
    ...
    render() {
        return React.createElement(
            'div',
            { id: 'GoalsApp', className: 'app' },
            React.createElement(GoalsHeading, { type: this.state.selectedTime, currentMonth: this.state.currentMonth, currentWeek: this.state.currentWeek, currentYear: this.state.currentYear }),
            React.createElement(GoalsForm, { addGoal: this.addGoal, selectedTime: this.state.selectedTime, changeType: this.handleChangeType }),
            this.state.requiredgoals.length > 0 ? this.state.requiredgoals.map(data => React.createElement(GoalsDisplay, { type: data.year ? 'years' : data.month ? 'months' : 'weeks', key: data.year || data.month || data.week, number: data.year || data.month || data.week, goals: data.goals, deleteGoal: this.deleteGoal, doneStateChange: this.doneStateChange })) : ''
        );
    }
}
