...
class GoalsApp extends React.Component {
    ...
    render() {
        return React.createElement(
            ...
            this.state.requiredgoals.length > 0 ? this.state.requiredgoals.map(data => React.createElement(GoalsDisplay, { type: data.year ? 'years' : data.month ? 'months' : 'weeks', key: data.year || data.month || data.week, number: data.year || data.month || data.week, goals: data.goals, doneStateChange: this.doneStateChange })) : ''
        );
    }
}
